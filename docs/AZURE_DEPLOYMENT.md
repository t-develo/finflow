...
| リソース | Dev 環境 | Prod 環境 |
|---------|---------|----------|
| App Service Plan | **無料 (F1)** | ~¥2,000 (B1) |
| Azure SQL Database | **~¥0〜¥300 (Serverless)** | ~¥750 (Basic) |
...
- **F1 (Free)**: CPU 60分/日の制限あり。アクティブな開発・テスト用途では十分。
- **Azure SQL Serverless**: 1時間無操作で自動停止。再起動時に~30秒かかる場合あり。
- **Dev 環境の削除**: 不要な場合は `az group delete --name finflow-dev-rg` でリソースグループごと削除できます。

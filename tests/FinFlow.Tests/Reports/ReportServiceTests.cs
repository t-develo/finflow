...
public class ReportServiceTests : IDisposable
{
    ...
    public async Task GetMonthlyReportAsync_WithExpenses_ReturnsCorrectTotals()
    {
        ...
        // EnsureCreated()でシードカテゴリ（ID=1:食費, ID=6:光熱費）が既に存在する
        var expenses = new List<Expense>();
        // 食費(categoryId=1): 20件 × 3250円 = 65000円
        for (int i = 0; i < 20; i++)
        {
            expenses.Add(BuildExpense(i + 1, TestUserId, 3250m, 2026, 3, 1 + (i % 28), categoryId: 1));
        }
        // 光熱費(categoryId=6): 3件 × 8333.33円 ≈ 25000円（合計をきれいにするため端数調整）
    ...
    {
        // Arrange: カテゴリ別に異なる金額を設定する（ID 100以上を使用してシードデータとの衝突を回避する）
        var category1 = BuildCategory(101, "テスト食費", "#F59E0B");
        var category2 = BuildCategory(102, "テスト交通費", "#3B82F6");
        var category3 = BuildCategory(103, "テスト娯楽", "#8B5CF6");
        ...
        {
            // 娯楽: 5000円（最少）
            BuildExpense(100, TestUserId, 5000m, 2026, 3, 1, categoryId: 103),
            // 食費: 30000円（最大）
            BuildExpense(101, TestUserId, 30000m, 2026, 3, 2, categoryId: 101),
            // 交通費: 10000円（中間）
            BuildExpense(102, TestUserId, 10000m, 2026, 3, 3, categoryId: 102),
        };
        await SeedExpensesAsync(expenses);
    ...
    public async Task GetMonthlyReportAsync_WithMultipleCategories_CalculatesPercentagesCorrectly()
    {
        ...
        // 光熱費: 25,000円 = 25.0%
        // その他: 40,000円 = 40.0%
        // ID 200以上を使用してシードデータとの衝突を回避する
        var cat1 = BuildCategory(201, "テスト食費2", "#F59E0B");
        var cat6 = BuildCategory(202, "テスト光熱費2", "#F97316");
        var cat8 = BuildCategory(203, "テストその他2", "#6B7280");
        _context.Categories.AddRange(cat1, cat6, cat8);
        await _context.SaveChangesAsync();

        var expenses = new List<Expense>
        {
            BuildExpense(200, TestUserId, 35000m, 2026, 3, 1, categoryId: 201),
            BuildExpense(201, TestUserId, 25000m, 2026, 3, 2, categoryId: 202),
            BuildExpense(202, TestUserId, 40000m, 2026, 3, 3, categoryId: 203),
        };
        await SeedExpensesAsync(expenses);
    ...
    {
        // Arrange: カテゴリマスタに色情報を設定する（ID 300以上を使用してシードデータとの衝突を回避）
        var category = BuildCategory(301, "テスト食費3", "#FF6384");
        _context.Categories.Add(category);
        await _context.SaveChangesAsync();

        var expense = BuildExpense(400, TestUserId, 5000m, 2026, 3, 1, categoryId: 301);
        ...
        breakdown[0].CategoryName.Should().Be("テスト食費3");
    }
